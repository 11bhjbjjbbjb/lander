...
class RampNode(Node):
    ...
    def __init__(self):
        ...
        self.posture_align_front_angle_deg = float(cfg.get('posture_align_front_angle_deg', 15.0))
        self.posture_align_front_center_deg = float(cfg.get('posture_align_front_center_deg', 180.0))
        ...
        self._front_angle_deg = self.posture_align_front_angle_deg
        self._front_center_deg = self.posture_align_front_center_deg
    ...
    def _reload_tolerance_config(self):
        ...
        if cfg is not None:
            ...
            self.posture_align_front_angle_deg = float(
                cfg.get('posture_align_front_angle_deg', self.posture_align_front_angle_deg)
            )
            self.posture_align_front_center_deg = float(
                cfg.get('posture_align_front_center_deg', self.posture_align_front_center_deg)
            )
            self._front_angle_deg = self.posture_align_front_angle_deg
            self._front_center_deg = self.posture_align_front_center_deg
    ...
    def start_posture_align_callback(self, request, response):
        ...
        self.get_logger().info(
            f'启动姿态摆正: front_center_deg={self._front_center_deg:.1f}, '
            f'front_angle_deg=±{self._front_angle_deg:.1f}'
        )
    ...
    def scan_callback(self, msg):
        ...
        if len(ranges) < 2:
            fallback_center_deg = self._front_center_deg + 180.0
            angles, ranges = _extract_front_scan(
                msg,
                half_angle_deg=self._front_angle_deg,
                front_center_deg=fallback_center_deg,
            )
            if len(ranges) >= 2:
                now = time.time()
                if now - getattr(self, '_last_scan_fallback_log', 0) >= 2.0:
                    self._last_scan_fallback_log = now
                    self.get_logger().warn(
                        f'姿态摆正主窗口无有效点，已回退到 center={fallback_center_deg:.1f}°'
                    )
        if len(ranges) < 2:
            self.cmd_pub.publish(Twist())
            return
    ...
    def up_callback(self, request, response):
        ...
        def _up_sequence():
            try:
                ...
                self._reload_tolerance_config()
                self.get_logger().info(
                    f'姿态摆正参数: front_center_deg={self._front_center_deg:.1f}, '
                    f'front_angle_deg=±{self._front_angle_deg:.1f}'
                )
                self.start_posture_align = True
                self._posture_align_stop_count = 0
